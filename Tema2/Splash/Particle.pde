...
void render(boolean useTexture)
{
    ...
    {
        imageMode(CENTER);
        image(_ps._tex,_s.x*FACTOR_CONVERSION,_s.y*FACTOR_CONVERSION,_radius*FACTOR_CONVERSION,_radius*FACTOR_CONVERSION);
    } 
    else
    {
        stroke(_color); 
        fill(_color); 
        circle(_s.x, _s.y, _radius);  // Dibujar un círculo
    }
}
}
