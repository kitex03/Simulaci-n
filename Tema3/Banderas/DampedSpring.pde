...
public class DampedSpring
{
   ...
   void update(float simStep)
   {
      if (_enabled)
      {
        ...
        _eN = d.copy().normalize();
        float deltaL =_l - d.mag() ;
        _l = d.mag();
         float fuerza_elastica = _Ke * (_l - _l0);
         _v = deltaL / simStep;
         
         float fuerza_amortiguacion = -_Kd * _v;
         PVector F_amort = PVector.mult(_eN.copy(), fuerza_amortiguacion);
         PVector F_elast = PVector.mult(_eN.copy(), fuerza_elastica);

         _F = PVector.add(F_elast, F_amort);
         applyForces();
      }
      //...
      //...
